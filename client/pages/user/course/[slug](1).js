...
const SingleCourse = () => {
  ...
  const loadCompletedLessons = async () => {
    const { data } = await axios.post(`/list-completed`, {
      courseId: course._id,
    });
  ...
  const markCompleted = async () => {
    const { data } = await axios.post(`/mark-completed`, {
      courseId: course._id,
  ...
  const markIncompleted = async () => {
    try {
      const { data } = await axios.post(`/mark-incomplete`, {
        courseId: course._id,
        lessonId: course.lessons[clicked]._id,
      });
      console.log(data);
        ...
        <div className="col">
          {clicked !== -1 ? (
            <>
              ...
              <MarkDownRenderer
                markdown={course.lessons[clicked].content}
